...
\subsection{Measures used to describe time series}
...
\begin{frame}
    ...
    \begin{block}{Note}
        For $s=t$ the covariance reduces to the variance, because
...
\begin{frame}
    ...
    \begin{proof}
		...
		\only<2> {
        We now show that Eq.~\ref{eq:ma} is the unique solution to Eq.~\ref{eq:ar}. Suppose $y_t$ is stationary and satisfies Eq.~\ref{eq:ar}, then
		\begin{align*}
			y_t &= \phi y_{t-1}+w_t\\
